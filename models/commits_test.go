...
func testCommitsExists(t *testing.T) {
	...
	e, err := CommitExists(ctx, tx, o.ID)
	if err != nil {
...
func testCommitsFind(t *testing.T) {
	...
	commitFound, err := FindCommit(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testCommitToOneRepoUsingRepo(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, repoDBTypes, false, repoColumnsWithDefault...); err != nil {
	...
	local.RepoID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Repo().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadRepo(ctx, tx, false, (*[]*Commit)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Repo == nil {
	...
	local.R.Repo = nil
	if err = local.L.LoadRepo(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Repo == nil {
		t.Error("struct should have been eager loaded")
...
func testCommitToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CommitSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Commit)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
		t.Error("struct should have been eager loaded")
...
func testCommitToOneSetOpRepoUsingRepo(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, repoDBTypes, false, strmangle.SetComplement(repoPrimaryKeyColumns, repoColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, repoDBTypes, false, strmangle.SetComplement(repoPrimaryKeyColumns, repoColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Repo{&b, &c} {
		err = a.SetRepo(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.RepoID != x.ID {
			t.Error("foreign key was wrong value", a.RepoID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.RepoID))
		reflect.Indirect(reflect.ValueOf(&a.RepoID)).Set(zero)
		...
		if a.RepoID != x.ID {
			t.Error("foreign key was wrong value", a.RepoID, x.ID)
		}
	}
}
func testCommitToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	commitDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `RepoID`: `integer`}
	_             = bytes.MinRead
)
