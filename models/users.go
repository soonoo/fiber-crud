...
type User struct {
	...
	GithubLogin string `boil:"github_login" json:"githubLogin" toml:"githubLogin" yaml:"githubLogin"`
	Email       string `boil:"email" json:"email" toml:"email" yaml:"email"`
	AvatarURL   string `boil:"avatar_url" json:"avatarURL" toml:"avatarURL" yaml:"avatarURL"`
...
var UserWhere = struct {
	ID          whereHelperint
	GithubLogin whereHelperstring
	Email       whereHelperstring
	AvatarURL   whereHelperstring
}{
	ID:          whereHelperint{field: "\"users\".\"id\""},
	GithubLogin: whereHelperstring{field: "\"users\".\"github_login\""},
...
var (
	userAllColumns            = []string{"id", "github_login", "email", "avatar_url"}
	userColumnsWithoutDefault = []string{"github_login", "email", "avatar_url"}
...
func (o *User) Commits(mods ...qm.QueryMod) commitQuery {
...
// Repos retrieves all the repo's Repos with an executor.
func (o *User) Repos(mods ...qm.QueryMod) repoQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"user_repos\" on \"repos\".\"id\" = \"user_repos\".\"repo_id\""),
		qm.Where("\"user_repos\".\"user_id\"=?", o.ID),
	)

	query := Repos(queryMods...)
	queries.SetFrom(query.Query, "\"repos\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"repos\".*"})
	}
...
func (userL) LoadCommits(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`commits`), qm.WhereIn(`commits.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadRepos allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRepos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("\"repos\".*, \"a\".\"user_id\""),
		qm.From("\"repos\""),
		qm.InnerJoin("\"user_repos\" as \"a\" on \"repos\".\"id\" = \"a\".\"repo_id\""),
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load repos")
	}

	var resultSlice []*Repo
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Owner, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for repos")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice repos")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on repos")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for repos")
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.Repos = append(local.R.Repos, foreign)
				if foreign.R == nil {
					foreign.R = &repoR{}
				}
...
func (o *User) AddCommits(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Commit) error {
	var err error
...
// AddRepos adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Repos.
// Sets related.R.Users appropriately.
func (o *User) AddRepos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Repo) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"user_repos\" (\"user_id\", \"repo_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
	...
	if o.R == nil {
		...
		o.R.Repos = append(o.R.Repos, related...)
	}
...
// SetRepos removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Repos accordingly.
// Replaces o.R.Repos with related.
// Sets related.R.Users's Repos accordingly.
func (o *User) SetRepos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Repo) error {
	query := "delete from \"user_repos\" where \"user_id\" = $1"
	...
	removeReposFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Repos = nil
	}
	return o.AddRepos(ctx, exec, insert, related...)
}

// RemoveRepos relationships from objects passed in.
// Removes related items from R.Repos (uses pointer comparison, removal does not keep order)
// Sets related.R.Users.
func (o *User) RemoveRepos(ctx context.Context, exec boil.ContextExecutor, related ...*Repo) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"user_repos\" where \"user_id\" = $1 and \"repo_id\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.Repos {
			...
			ln := len(o.R.Repos)
			if ln > 1 && i < ln-1 {
				o.R.Repos[i] = o.R.Repos[ln-1]
			}
			o.R.Repos = o.R.Repos[:ln-1]
			break
		}
	}
...
func removeReposFromUsersSlice(o *User, related []*Repo) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
