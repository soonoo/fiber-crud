...
type Commit struct {
	...
	RepoID int `boil:"repo_id" json:"repoID" toml:"repoID" yaml:"repoID"`
...
var CommitWhere = struct {
	...
	RepoID whereHelperint
}{
	ID:     whereHelperint{field: "\"commits\".\"id\""},
	UserID: whereHelperint{field: "\"commits\".\"user_id\""},
	RepoID: whereHelperint{field: "\"commits\".\"repo_id\""},
}
...
var (
	commitAllColumns            = []string{"id", "user_id", "repo_id"}
	commitColumnsWithoutDefault = []string{"user_id", "repo_id"}
	commitColumnsWithDefault    = []string{"id"}
	commitPrimaryKeyColumns     = []string{"id"}
)
...
// Repo pointed to by the foreign key.
func (o *Commit) Repo(mods ...qm.QueryMod) repoQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.RepoID),
	}
...
func (o *Commit) User(mods ...qm.QueryMod) userQuery {
	queryMods := []qm.QueryMod{
...
// LoadRepo allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (commitL) LoadRepo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCommit interface{}, mods queries.Applicator) error {
	var slice []*Commit
	var object *Commit
	...
	if singular {
		...
		args = append(args, object.RepoID)
	...
	query := NewQuery(qm.From(`repos`), qm.WhereIn(`repos.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load Repo")
	}

	var resultSlice []*Repo
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice Repo")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for repos")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.RepoID == foreign.ID {
				local.R.Repo = foreign
				if foreign.R == nil {
					foreign.R = &repoR{}
				}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (commitL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCommit interface{}, mods queries.Applicator) error {
	var slice []*Commit
	var object *Commit
...
// SetRepo of the commit to the related item.
// Sets o.R.Repo to related.
// Adds o to related.R.Commits.
func (o *Commit) SetRepo(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Repo) error {
	var err error
	if insert {
...
// SetUser of the commit to the related item.
// Sets o.R.User to related.
// Adds o to related.R.Commits.
func (o *Commit) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
...
func FindCommit(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Commit, error) {
	commitObj := &Commit{}
	...
	query := fmt.Sprintf(
		"select %s from \"commits\" where \"id\"=$1", sel,
	)
...
func (o *Commit) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"commits\" WHERE \"id\"=$1"
...
func (o *Commit) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindCommit(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func CommitExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"commits\" where \"id\"=$1 limit 1)"
