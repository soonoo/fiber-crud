...
func testUserToManyCommits(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCommits(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRepos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, repoDBTypes, false, repoColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, repoDBTypes, false, repoColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	_, err = tx.Exec("insert into \"user_repos\" (\"user_id\", \"repo_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_repos\" (\"user_id\", \"repo_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.Repos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadRepos(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Repos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Repos = nil
	if err = a.L.LoadRepos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Repos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpCommits(t *testing.T) {
	var err error
...
func testUserToManyAddOpRepos(t *testing.T) {
	var err error
	...
	foreigners := []*Repo{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, repoDBTypes, false, strmangle.SetComplement(repoPrimaryKeyColumns, repoColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Repo{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddRepos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Repos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Repos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Repos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpRepos(t *testing.T) {
	var err error
	...
	err = a.SetRepos(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetRepos(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRemoveOpRepos(t *testing.T) {
	var err error
	...
	err = a.AddRepos(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveRepos(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `GithubLogin`: `text`, `Email`: `text`, `AvatarURL`: `text`}
	_           = bytes.MinRead
)
